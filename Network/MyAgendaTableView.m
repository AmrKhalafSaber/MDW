...
@interface MyAgendaTableView ()

@end

@implementation MyAgendaTableView
{
NSString *status;
...
-(void)getalldaysbeansmethod:(AllDays *)alldays
{
    ...
    // int i=0;
    for (Agenda *showagenda in alldays.includesAgendas) {
        NSLog(@"showagenda.includesSessions%d",[showagenda.includesSessions count]);
        for (Session *singlesession in showagenda.includesSessions) {
            //NSLog(@"%@",singlesession.name);
            // NSLog(@"number of showagenda.includesSessions %d",[showagenda.includesSessions count]);
            NSLog(@"liked %@",singlesession.liked);
            if ([singlesession.liked intValue]==1) {
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [_allmyagenda setDelegate:self];
    [_allmyagenda setDataSource:self];
    //request for json
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyAgendaAllViewCell" forIndexPath:indexPath];
